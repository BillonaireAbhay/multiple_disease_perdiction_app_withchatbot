...
symptoms_dict = { ... }  # truncated here for brevity, keep as is in your code
diseases_list = { ... }  # truncated here for brevity, keep as is in your code

# Helper function to extract info for a disease
def helper(dis):
...
# ─── Symptom ↔ Index and Disease List ──────────────────────────────
...
# ─── Helper to gather text, precautions, meds, diets, workouts ─────
def helper(disease_name):
    desc = description.loc[description['Disease']==disease_name, 'Description'].str.cat(sep=' ')
    ...
    med  = medications.loc[medications['Disease']==disease_name, 'Medication'].tolist()
    die  = diets.loc[diets['Disease']==disease_name, 'Diet'].tolist()
    wrk  = workout.loc[workout['disease']==disease_name, 'workout'].tolist()
    return desc, pre, med, die, wrk


# ─── Prediction function, now skips unknown symptoms ───────────────
def get_predicted_value(patient_symptoms):
    ...
    if unknown:
        app.logger.warning(f"Ignored unrecognized symptoms: {unknown}")

    raw_pred = svc.predict([input_vector])[0]
    return diseases_list.get(raw_pred, "Unknown Disease")
...
def predict():
    ...
    user_symptoms = [s.lower().strip("[] '\"") for s in symptoms.split(',')]
    predicted = get_predicted_value(user_symptoms)
    desc, precs, meds, diets_listed, workouts = helper(predicted)

    return render_template("index.html",
                           predicted_disease=predicted,
                           dis_des=desc,
                           my_precautions=precs,
                           medications=meds,
                           my_diet=diets_listed,
                           workout=workouts)
...
if __name__ == "__main__":
    # ensure templates and static are found relative to this file
    app.run(debug=True)
